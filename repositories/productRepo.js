...
const get = (options) => {
    const {size,page,sort,search,direction} = options; // property destructure - we are extracting all the attributes of object 'option'
    // and then assigning to the variable name which is mentioned, and both the variable names are same.
    const rowsToSkip = (page - 1) * size; // page = current page
    const filter = getFilterExp(search);
